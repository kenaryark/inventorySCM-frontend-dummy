...
export default function LazyLoadDemo() {
  ...
  const [opsiMultiselectKodeMaterial, setOpsiMultiselectKodeMaterial] =
    useState();
  const [opsiMultiselectNamaMaterial, setOpsiMultiselectNamaMaterial] =
    useState();
  const [opsiMultiselectKodeMaterialGrup, setOpsiMultiselectKodeMaterialGrup] =
    useState();
  ...
  const statusesName = [
    ...new Set(customers?.map((item) => item.statusProjek)),
  ];
  ...
  const [
    visibleLabelMultiselectKodeMaterialGrup,
  ...
  const onGlobalFilterChange = (e) => {
    ...
    if (visibleLabelMultiselectNamaMaterial.length !== 0) {
      const duplicatedArray = visibleLabelMultiselectNamaMaterial.map(
        (item) => ({
  ...
  const handleKodeMaterialGrupToggle = (event) => {
    ...
    const constraintValues = selectedColumns.map(
      (item) => item.kodeMaterialGrup
    );
    filter = {
      operator: FilterOperator.OR,
      constraints:
  ...
  const downloadfile = async () => {
    try {
      const response = await fetch(
        "http://localhost:4001/api/v1/inventory/export",
        {
          method: "GET",
        }
      );
      ...
      const filenameMatch =
        contentDisposition && contentDisposition.match("products_export.xlsx");
  ...
  const downloadfileFilter = async () => {
    try {
      const response = await CustomerService.getFilterDataExportExcel({
  ...
  useEffect(() => {
    CustomerService.getTotalMaterialQty({
      lazyEvent: JSON.stringify(lazyState),
    }).then((data) => {
      const formatDecimal = parseFloat(data.subTotalValueInventoryQty).toFixed(
        3
      );
      const formattedValue = parseFloat(formatDecimal).toLocaleString("id-ID", {
        style: "currency",
        currency: "IDR",
  ...
  const loadLazyData = () => {
    ...
    try {
      ...
      networkTimeout = setTimeout(() => {
        CustomerService.getDataInventory({
          lazyEvent: JSON.stringify(lazyState),
        }).then((data) => {
          ...
          data.totalRecords
            ? true
            : showMessage("Warning", "Data Not Found", "warn");
          ...
          const materialGroup = data.opsiMultiselect.kodeMaterialGrup.map(
            (item) => {
